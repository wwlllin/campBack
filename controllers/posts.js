...
export const get = async (req, res) => {
  try {
    ...
    const total = await posts.countDocuments({ release: true })
    res.status(StatusCodes.OK).json({
      success: true,
      message: '',
      result: {
...
export const edit = async (req, res) => {
  try {
    ...
    req.body.image = req.files.map(file => file.path)
    await posts.findByIdAndUpdate(req.params.id, req.body, { runValidators: true }).orFail(new Error('NOT FOUND'))
...
export const remove = async (req, res) => {
  try {
    ...
    await posts.findByIdAndDelete(req.params.id).orFail(new Error('NOT FOUND'))
